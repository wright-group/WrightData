...
def workup_movie():
    ...
    processed_movie.save('processed_movie.p')
    # finish
    return raw_movie, processed_movie

# get from pickle or create
if os.path.isfile('raw_movie.p'):
    raw_movie = wt.data.from_pickle('raw_movie.p', verbose=False)
    processed_movie = wt.data.from_pickle('processed_movie.p', verbose=False)
else:
...
def workup_absorbance():
    ...
    absorbance_data.save('absorbance_data.p')
    return absorbance_data
    
# get from pickle or create
if os.path.isfile('absorbance_data.p'):
    absorbance_data = wt.data.from_pickle('absorbance_data.p', verbose=False)
else:
